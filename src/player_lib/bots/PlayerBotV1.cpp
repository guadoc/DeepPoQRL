...
void PlayerBotV1::init_macro_params(default_random_engine& generator, float learning_rate_base, float param_reg_base){
	std::normal_distribution<double> distribution_lr(learning_rate_base,0.1);
	this->learning_rate = (float)distribution_lr(generator);
	std::normal_distribution<double> distribution_reg(param_reg_base,0.1);
...
void PlayerBotV1::init_macro_params(default_random_engine& generator){
	std::normal_distribution<double> distribution_lr(0.01,0.01);
	this->learning_rate = (float)distribution_lr(generator);
	std::normal_distribution<double> distribution_reg(0.01,0.01);
	this->param_reg = (float)distribution_reg(generator);
...
void PlayerBotV1::mute_macro_params(){
	std::default_random_engine generator;
	this->init_macro_params(generator, this->learning_rate, this->param_reg);
}
...
void PlayerBotV1::mute_macro_params(list<AbstractPlayer*> & winning_players, default_random_engine& generator){
	...
	float lr = static_cast<PlayerBotV1*>(*it)->get_learning_rate();
	rand_index = rand() % winning_players.size();
	it = winning_players.begin();
	std::advance(it, rand_index);
	float param_reg = static_cast<PlayerBotV1*>(*it)->get_param_reg();
	this->init_macro_params(generator, lr, param_reg);
}
